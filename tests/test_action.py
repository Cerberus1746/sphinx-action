...
TEST_PROJECTS_DIR = os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_projects")


default_rst = "index.rst"
...
class TestAction(unittest.TestCase):
    ...
    def test_extract_line_information_with_no_line_info(self):
        self.assertEqual(
            ...
            ("warnings_and_errors{}{}".format(os.sep, default_rst), 1),
        )
    ...
    def test_extract_line_information_with_invalid_lineno(self):
        self.assertEqual(action.extract_line_information(f"{default_rst}:a:"), None)
    ...
    def test_build_docs_warnings(self):
        ...
        self.assertTrue(annotations[1].path.endswith(default_rst))
        self.assertEqual(annotations[1].start_line, 24)
        self.assertEqual(annotations[1].end_line, 24)
        self.assertEqual('Unknown directive type "BADDIRECTIVE".', annotations[1].message)
        ...
        self.assertEqual("document isn't included in any toctree", annotations[2].message)
